...
export default function VideoCta() {
  return (
    <section className="relative w-full min-h-svh overflow-hidden bg-toursl-sand">
      ...
      <div className="relative z-[2] w-full max-w-[1360px] mx-auto px-20 pt-[15vh] max-md:px-6 max-md:pt-[10vh] flex flex-col items-center text-center">
        <h2 className="font-display text-[clamp(34px,5.4vw,74px)] leading-[1.12] tracking-[-0.01em] text-toursl-text max-w-[900px] mb-7">
          <WaveText text="A short stay." />
          <br className="max-md:hidden" />
          <WaveText text="A very long list." />
        </h2>

        <p className="font-sans text-lg font-medium text-toursl-text/75 leading-relaxed max-w-[560px] mb-10 text-balance">
        ...
        <div className="flex flex-wrap items-center justify-center gap-4">
          <button
            type="button"
            className="inline-flex h-14 w-[210px] max-md:w-[190px] items-center justify-center bg-toursl-dark text-[#fafafa] border border-toursl-dark cursor-pointer font-sans text-[15px] font-medium uppercase tracking-[0.04em] rounded-full transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] hover:bg-[#333] hover:scale-[1.03] active:scale-95"
          >
            Plan My Trip
          </button>
          <button
            type="button"
            className="inline-flex h-14 w-[210px] max-md:w-[190px] items-center justify-center bg-white/40 text-toursl-text border border-toursl-text/25 cursor-pointer font-sans text-[15px] font-medium uppercase tracking-[0.04em] rounded-full backdrop-blur-[14px] transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] hover:bg-white/70 hover:scale-[1.03] active:scale-95"
          >
            Publish a Tour
          </button>
        </div>
